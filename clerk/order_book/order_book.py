...
class OrderBook:
    ...
    def export(self, output_file):
        ...
        curs = conn.cursor()
        curs.execute("create table __transactions__ (rowid integer primary key, date integer, description text, isin text, quantity integer, price real, value real, fee real, capital_returns_tax real, soli real, church_tax real, path text)")
        curs.executemany("insert or ignore into __transactions__ (date, description, isin, quantity, price, value, fee, capital_returns_tax, soli, church_tax, path) values(?,?,?,?,?,?,?,?,?,?,?)", sorted(self.transactions))
        conn.commit()
        conn.close()
